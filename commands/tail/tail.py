...
desc = """Print the last 10 lines of each FILE to standard output.  With more than one FILE, precede each with a header giv‐
       ing the file name.
...
parser.add_argument(
    '-n', '--lines', help="print the last NUM lines instead of the first 10", default=10, type=int)
...
class Text:
    def __init__(self: object, text: str) -> None:
        self.text = text
        self.nlines = self.get_last_n_lines(args.lines)

    def get_last_n_lines(self, n) -> str:
